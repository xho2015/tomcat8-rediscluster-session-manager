...
public class RedisClusterSessionManager extends ManagerBase implements Lifecycle {
  ...
  protected RedisClusterSessionHandlerValve handlerValve;
  ...
  */
  ...
  protected synchronized void startInternal() throws LifecycleException {
    ...
    for (Valve valve : getContainer().getPipeline().getValves()) {
      if (valve instanceof RedisClusterSessionHandlerValve) {
        this.handlerValve = (RedisClusterSessionHandlerValve) valve;
        this.handlerValve.setRedisSessionManager(this);
        log.info("Attached to RedisClusterSessionHandlerValve");
  ...
  public Session createSession(String requestedSessionId) {
    ...
    //try {
      ...
      if (null != requestedSessionId) {
        ...
        } while (jedisCluster.setnx(sessionId.getBytes(), NULL_SESSION) == 0L); // 1 = key set; 0 = key already existed
      }
      ...
      if (null != session) {
        try {
          error = saveInternal(jedisCluster, session, true);
  ...
  */
  ...
  public void save(Session session, boolean forceSave) throws IOException {
    ...
    try {
      //jedis = acquireConnection();
      error = saveInternal(jedisCluster, session, forceSave);
    } catch (IOException e) {
      throw e;
    } /*finally {
      if (jedis != null) {
  ...
  private Set<HostAndPort> parseHostAndPort() throws Exception {
    try {
      if (redisNodes == null || redisNodes.length() == 0) {
        throw new RuntimeException("redis nodes config error");
      }
      ...
      throw new Exception("unable to read jedis config", ex);
    }
  }
  ...
  private void initializeDatabaseConnection() throws LifecycleException {
    try {
      ...
      this.connectionPoolConfig.setMaxWaitMillis(getMaxWaitMillis());
  ...
  private void initializeSerializer() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
    ...
    //HXY: Add below section to get loader
    ...
    //HXY: Remove below to adapt to tomcat ver.8
    /*
...
//HXY:comment it out since original manager have defined this
/*
